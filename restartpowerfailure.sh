...
function install_script {
		...
		## /usr/local/bin/
		## then just set this function to 'return'
		##
		# return
		##
		## if you want it installed somewhere else
		## change the '/usr/local/bin/' portion of 'INSTALL_TO=' below


		INSTALL_TO="/usr/local/bin/$0:t"

		if [[ "$INSTALL_TO:h" != "$0:h" ]]
		then
				## If we get here, we are not installed where we should be

			if [[ -e "$INSTALL_TO" ]]
			then
				log "$0 does not appear to be installed at its expected location of $INSTALL_TO, but $INSTALL_TO exists. I don't know what to do. Giving up"
				return
			fi
